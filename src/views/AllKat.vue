...
async function getVoucherData() {
  try {
    const response = await axios.get('http://127.0.0.1:8000/api/voucher?status=unclaimed')
...
<template>
  <div class="container">
    ...
    <div v-if="voucher && voucher.length" class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <VoucherCard v-for="voc in voucher" :key="voc.id" :nama="voc.nama" :kategori="voc.kategori" :foto="voc.foto" />
    </div>
